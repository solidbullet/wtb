...
func Load() *Config { return &Config{Port: getEnvInt("PORT", 8089), DSN: getEnv("DATABASE_DSN", "host=/tmp user=admin dbname=wtb_analytics sslmode=disable TimeZone=Asia/Shanghai"), JWTSecret: getEnv("JWT_SECRET", "dev-secret-change-in-production")} }
